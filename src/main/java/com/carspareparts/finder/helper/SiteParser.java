...
public class SiteParser {
    ...
    private List<SparePart> extractFromElementList(Element element, String className) {
        List<SparePart> sparePartList = new ArrayList<>();
        try {
            Elements elementNames = element.getElementsByTag(BusinessNameHolder.A.getPath());
            Elements elementDescriptions = element.getElementsByTag(BusinessNameHolder.STRONG.getPath());
            int descriptionIndex = 0;
            for (int i = 0; i < elementNames.size() - 1; i += 2) {
                String url = elementNames.get(i).attr(BusinessNameHolder.HREF.getPath());
